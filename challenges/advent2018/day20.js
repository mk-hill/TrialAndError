...
const directionsString = input.day20;
...
function generateMap(directions = directionsString) {
  let currentNode = new Node(0, 0, 0);
  const path = [currentNode];
  const map = {};

  [...directions].forEach((char) => {
    if (char === '(') path.push(currentNode);
...
const distances = [].concat(
  ...Object.values(generateMap()).map(y => Object.values(y).map(node => node.dist)),
);

console.group('Day 20');
console.log(`Farthest room is \x1b[32m${Math.max(...distances)}\x1b[0m doors away.`); // 3502
