...
const [[caveDepth], targetCoords] = input.split('\n').map(line => line
  .split(' ')[1]
  .split(',')
  .map(str => Number(str)));

// const [testDepth, testCoords] = [510, [10, 10]];

const TORCH = 'torch';
const GEAR = 'climbingGear';
...
const map = {
  ...
  getGeologicIndex(x, y, [targetX, targetY] = targetCoords) {
    if (this.regions[y] && this.regions[y][x]) return this.regions[y][x].geologicIndex;
    if (x === 0 && y === 0) return 0;
    if (x === targetX && y === targetY) return 0;
    if (y === 0) return x * 16807;
    if (x === 0) return y * 48271;
    return this.getErosionLevel(x - 1, y) * this.getErosionLevel(x, y - 1);
  },

  getErosionLevel(x, y, depth = caveDepth) {
    if (this.regions[y] && this.regions[y][x]) return this.regions[y][x].erosionLevel;
    return (this.getGeologicIndex(x, y) + depth) % 20183;
  },

  addRegion(x, y) {
    if (!this.regions[y]) this.regions[y] = {};
    const erosionLevel = this.getErosionLevel(x, y);
    const type = this.types[erosionLevel % 3];
    this.regions[y][x] = {
      x,
      y,
      geologicIndex: this.getGeologicIndex(x, y),
      erosionLevel,
      ...type,
    };
  },
  ...
  getRegion(x, y) {
    if (!this.regions[y] || !this.regions[y][x]) this.addRegion(x, y);
    return this.regions[y][x];
  },
};

function calculateTotalRisk([targetX, targetY] = targetCoords) {
  let totalRisk = 0;
  for (let x = 0; x <= targetX; x++) {
    for (let y = 0; y <= targetY; y++) {
      totalRisk += map.getRegion(x, y).risk;
    }
  }
  return totalRisk;
}

console.group('Day 22');

console.log(
  `Total risk level for area between 0,0 and target is \x1b[32m${calculateTotalRisk()}\x1b[0m.`,
); // 11575

/**
 * Your puzzle answer was 11575.
...
function getMovementResults(currentToolOptions, destination) {
  ...
  return {
    ...destination,
    moveCost: commonTools.length ? 1 : 8,
    newTools: commonTools.length ? commonTools : destination.tools,
    distanceToTarget: calculateDistanceToTarget([destination.x, destination.y]),
  };
}
...
// todo fix logic here
function findLowestCostMove(x, y, currentToolOptions) {
  let options = checkSurroundings(x, y, currentToolOptions).sort((a, b) => a.moveCost - b.moveCost);
  options = options.filter(option => option.moveCost === options[0].moveCost);
  if (options.length === 1) return options[0];
  options = options
    .sort((a, b) => a.distanceToTarget - b.distanceToTarget)
    .filter(option => option.distanceToTarget === options[0].distanceToTarget);
  if (options.length === 1) return options[0];
  // todo decide option if more remain
  return options[0];
}
