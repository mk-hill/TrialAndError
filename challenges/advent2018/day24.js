...
const [immuneGroupStats, infectionGroupStats] = input.day24.split('\n\n').map(str => str
  .split('\n')
  .slice(1)
  .map(line => line.split(' '))
  .map((ar) => {
    ...
    return {
      units: Number(ar[0]),
      hp: Number(ar[4]),
      ap: Number(ar[ar.indexOf('does') + 1]),
      damageType: ar[ar.indexOf('does') + 2],
      initiative: Number(ar[ar.length - 1]),
      weaknesses: tally.weak,
      immunities: tally.immune,
    };
  }));
...
class Group {
  ...
  getTargetChoice(enemies) {
    let enemyValues = enemies
      .map((enemy, index) => ({
        index,
        potentialDamage: enemy.getDamageValue(this.ep, this.damageType),
        ep: enemy.ep,
    ...
    ) return enemies[enemyValues[0].index];
    enemyValues = enemyValues
      .filter(enemy => enemy.potentialDamage === enemyValues[0].potentialDamage)
      .sort((x, y) => y.ep - x.ep);
    if (enemyValues[0].ep !== enemyValues[1].ep) return enemies[enemyValues[0].index];
    return enemies[
      enemyValues
        .filter(enemy => enemy.ep === enemyValues[0].ep)
        .sort((x, y) => y.initiative - x.initiative)[0].index
    ];
  }
  ...
  getDamageValue(val, type) {
    if (val < 1 || this.immunities.has(type)) return 0;
    if (this.weaknesses.has(type)) return val * 2;
    return val;
  }
}
// Get total units from array of Groups
const getTotalUnits = ar => ar.reduce((totalUnits, group) => {
...
function battle(boost = 0, immuneStats = immuneGroupStats, infectionStats = infectionGroupStats) {
  ...
  let infectionArmy = infectionStats.map(stats => new Group({ type: 'infection', ...stats }));
  const unitsRemaining = [[getTotalUnits(immuneArmy)], [getTotalUnits(infectionArmy)]];

  const getChoiceOrder = () => immuneArmy
    .concat(infectionArmy)
    .sort((x, y) => y.ep * 100 + y.initiative - (x.ep * 100 + x.initiative));
  const getAttackOrder = () => immuneArmy.concat(infectionArmy).sort((x, y) => y.initiative - x.initiative);

  while (immuneArmy.length && infectionArmy.length) {
    getChoiceOrder().forEach(group => group.acquireTarget(group.type === 'immune' ? infectionArmy : immuneArmy));
    getAttackOrder().forEach(group => group.attack());

    immuneArmy = immuneArmy.filter(group => !group.isDead);
    infectionArmy = infectionArmy.filter(group => !group.isDead);
    ...
    ) return { boost, winner: 'stalemate' };
  }

  const remainingArmy = immuneArmy.length ? immuneArmy : infectionArmy;
  return {
    boost,
    winner: remainingArmy[0].type,
    unitsLeft: getTotalUnits(remainingArmy),
  };
}

console.group('Day 24');
const p1 = battle();
console.log(`${p1.winner} won with \x1b[32m${p1.unitsLeft}\x1b[0m units left.`); // 21199

/**
 * Your puzzle answer was 21199.
...
function getMinWinningBoost() {
  let boost = 1;
  let result = battle(1);
  while (result.winner !== 'immune') {
    result = battle(boost);
    boost += 1;
  }
  return result;
...
const p2 = getMinWinningBoost();

console.log(
  `Minimum boost of ${p2.boost} results in ${p2.winner} winning with \x1b[32m${
    p2.unitsLeft
  }\x1b[0m units left.`,
); // 5761

console.groupEnd('Day 24');
